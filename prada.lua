...
function GoUnder(Value)
    ...
    if Value and not SideBar:GetAttribute("WasVisible") and not IsUnderground then
        IsUnderground = false
        repeat task.wait() until (LocalRoot and LocalHead and LocalHumanoid and MainUI.Enabled)
        task.wait(0.25)
        ...
        if Tries >= 3 then
            ...
            if GetValue("OfficialGame") then
                StarterGui:SetCore("SendNotification",
                    {
                        Title = "Fail",
                        Text = "Failed to become invincible, if this keeps happning please report this in the discord server",
                ...
                StarterGui:SetCore("SendNotification",
                    {
                        Title = "Fail",
                        Text = "Failed to become invincible, this feature is not supported here",
                        Icon = "rbxasset://textures/DevConsole/Warning.png",
                        Duration = 4.5
                    }
                )
                FeatureLoadout["Features"]["Invincible"]["Instance"]:SetAttribute("Requirement", true)
            end
...
FeatureLoadout["ExploitFunctions"]["TabAttributes"]["DisplayTitle"] = string.format(
    "Script Made by %s | %s    ",
    RichTextGradientColor("Naiko Exploits",{Color3.fromRGB(0, 255, 128),Color3.fromRGB(0, 102, 255)}),
...
PlayerGui.ChildAdded:Connect(function(Child)
    if Child.Name == "PuzzleUI" then
        ...
        ClonedLoadUI.Visible = GetValue("AutoGeneratorPuzzle")
        FeatureLoadout["Automation"]["AutoGeneratorPuzzle"]["Instance"]:GetPropertyChangedSignal("Value"):Connect(function()
            ClonedLoadUI.Visible = GetValue("AutoGeneratorPuzzle")
        end)
        task.spawn(function()
